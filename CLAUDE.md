...
- **No stray values:** no raw type sizes, leading, letter-spacing, colours or spacings in `src/`. Snap to a token; add one only for a repeated role with no fit (then add the Figma variable too). Run the token audit in [docs/08 §7](docs/08-page-review-checklist.md) before handover.
