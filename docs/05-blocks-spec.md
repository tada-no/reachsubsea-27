...
- **Figures · Layout Cards (review rounds 3–4, Q61):** heading, then the value points as tint cards stacked in cols 1–7. Each card's reading order is title (H4) → body → proof figure (36 → 44, right-aligned, counts up) with its caption. The split point becomes a Navy feature card (`bg/navy-800`) in cols 8–12. Title → donut (up to 368px, with the card's inner gaps at 24 / 16, the largest that keeps the card within the value cards' height; arcs drawn as solid trim paths with **no gaps**, so a 100% split reads as a full ring; navy/200 for the counterpart, so the sage/400 lead share pops; navy/400 read as disabled) → labels in a row under the ring, each on its own arc's side (H4 value, then a dot and a small name) → body. The centre shows the period in bold (`Body/Lead`) over the measure ("Q2 2026 / Revenue by segment"). There is no legend or caption. Stacks below 1100; on phones the figure drops under the text.
