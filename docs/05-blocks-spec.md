...
_**21 Sep 2026 (Q67): three blocks split for editors.** Card grid's Bento → **Card bento** (§2.2b); Split media's Figures and Embed → **Figures** (§2.4b) and **Split embed** (§2.4c), Card media dropped; Stats band's Results → **Results band** (§2.5b). Nothing changed visually (checked element by element on the seven built pages at 1440 · 1100 · 800 · 375). **Figma built 21 Sep 2026** (page Blocks `12:25`), along with the two custom visuals that were also missing, Lifecycle and Values. Block count in code: 22 (docs/09 §4)._
...
| Budget | **21 Sep 2026 (Q67): the count is no longer the goal, editor safety is.** A block with several media types or styles whose fields only apply to one is split into simple blocks (Card bento, Figures, Split embed, Results band). Original budget: 14 custom blocks. Long-form copy (news body, legal pages, project write-ups) uses **core blocks** (Heading, Paragraph, List, Image, Table, Buttons) in the 880 column, styled by theme.json. Those don't count toward the 14. **17 Sep 2026:** Live operations (§2.15) was added as a 15th block, outside this budget, per explicit user request after the client PDF review raised it — see [07-live-operations.md](07-live-operations.md). |
...
## 2. The 14 blocks (plus §2.15, added 17 Sep 2026, and §2.16, added 22 Sep 2026)
...
**22 Sep 2026 (Q74):** right fade off up to 1240 (no edge to hide). Below 900 the photo is a top band (`min(90vw, 560px)` tall) fading into a solid navy/900 panel that holds the copy; the title starts at 70% of the band, the breadcrumb stays pinned. On phones the lead sets in Body. Card grids with pictogram media open into hairline rows below 600; Image bg cards put the photo in a 4:3 band over a navy/900 text panel below 600. Photo hero min-height `clamp(640px, 47.2vw, 800px)` (680 at 1440). Above 1240 the photo keeps a centred 1240 photo's left edge and grows right to the screen edge, max 1600 (the one place a photo is enlarged, ≤1.29×), with a 30% right fade into navy; left fade (372) and copy ellipse stay 1240-based. *Figma updated 22 Sep 2026 (ledger `phase4MobileHeroScrim22Sep`).*
...
### 2.2b Card bento · `reach/card-bento` (split out of Card grid, 21 Sep 2026, Q67)
`Pattern` lead-quad · lead-tall-wide · lead-tall-trio-a · lead-tall-trio-b · trio-wide × `Breakpoint` = **10**; `Show section header`. Fields: `sectionHeader`, `pattern`, `cards[]` (one per cell, in order). Figma: `Block/Card bento` `312:10744`, doc frame `310:4981` (built 21 Sep 2026; Mobile is one 327 stack per pattern).
- A grid where the cards differ: image, stat, news and event cards tile into one composition on the 12-column grid. Each card keeps its own Media and Surface.
- **The editor picks a pattern and fills its cells in order; a card has no column or row span** (the old per-card `span` is gone). Patterns live in `src/data/bento-patterns.ts` and were lifted from the five approved bentos: Investors (lead-quad, 5 cards), Subsea (lead-tall-wide, 6), Home (lead-tall-trio-a, 7), Services (lead-tall-trio-b, 7), Careers (trio-wide, 5). Card count must equal the pattern's cell count (extra cards are dropped, a short list warns at build).
- Tablet 600–1199: 6 columns (wide cards full width, the rest halves, one row tall; if that leaves a hole the last single-row wide card drops to a half, else the last single-row card goes full width). Mobile: one column. One-row image cards get a left-to-right scrim; phones go back to the bottom ramp. Tiles settle into their cell on reveal (scale, no rise).
- WordPress: a `pattern` select with a preview of each shape, and a card repeater whose min and max rows equal the pattern's cell count.
...
**21 Sep 2026 (Q67): split.** Figures, Card and Embed are no longer media options of this block, so every field applies to what the editor picked. Figures is **§2.4b**, the share-graph embed is **§2.4c** Split embed, and the Card media (unused on every built page) was dropped: a Card in a section is Card grid or Card bento. What stays here: Image · Video · Spec table · Numbered list, layouts Split · Wide · Stacked (Numbered list only), `quote`, `pictogram`.
- Layout **Stacked** (Numbered list, review rounds 2 and 4): heading (max 880), then `items` as 3 numbered tint cards (number above title; subgrid rows so the text lines up across cards; 1 column below 900).
...
### 2.4b Figures · `reach/figures` (split out of Split media, 21 Sep 2026, Q67)
`Layout` Sticky · Cards × `Breakpoint` = **4**; `Title`, `Show eyebrow`. Fields: eyebrow, title, `layout`, `figures[]` (kicker, title, text, figure or `split[]`, caption, period). Figma: `Block/Figures` `316:6183`, doc frame `314:6177` (built 21 Sep 2026; uses the new `Heading/Display XL` and `Heading/Figure` text styles, and the donut is two arc ellipses).
- **Layout Sticky (18 Sep 2026, Why invest):** 2–4 points, each with a proof figure (value, or a two-part split bar such as the revenue mix) and a caption. The H2 spans cols 1–8. The figure column (1–5) is sticky and wipes to the point crossing the middle of the viewport (clip + rise, no fade). The points (kicker, H3, lead) scroll in cols 7–12, min-height 400. Mobile: no sticky, each point shows its figure inline. Best on Navy.
- **Layout Cards (review rounds 3–4, Q61):** heading, then the value points as tint cards stacked in cols 1–7. Each card's reading order is title (H4) → body → proof figure (36 → 44, right-aligned, counts up) with its caption. The split point becomes a Navy feature card (`bg/navy-800`) in cols 8–12. From 1280 up: title → donut → labels → body. The value cards alone set the row height, and the donut fills the height left over (flex 1 with size containment; the ring is the largest square that fits, max 400). The arcs are solid with **no gaps**, drawn by one masked trim-path sweep clockwise (1.4s ease-in-out); navy/200 is the counterpart and sage/400 the lead share. Under the ring the labels sit on their own arcs' sides (H4 value, then a dot and a small name). The centre shows the period in bold (`Body/Lead`) over the measure. There is no legend or caption. **Round 7:** below 1280 the 5-column card is too narrow, so the feature card spans the full width under the value cards. **768–1279:** one reading column on the left (title → body → the two shares side by side) with the donut on the right (280–360), both vertically centred. **Below 768:** stacked, with the ring as wide as the card. The sweep mask uses `userSpaceOnUse` over the full viewBox; the default bounding-box region clipped the ring's outer edge.
- Review round 1: the swap between points is directional (the old figure exits up in 0.32s, the new one rises in with a 0.14s delay), and the points have no rules.
- Numbers in the figures are proof points quoted from the Latest results / Key figures options pages, never retyped (docs/05 §0).

### 2.4c Split embed · `reach/split-embed` (split out of Split media, 21 Sep 2026, Q67)
`Position` End · Start × `Breakpoint` = **4**; `Title`, `Body`, `Show eyebrow`, `Show actions`. Fields: eyebrow, title, `headingLevel`, `body`, `actions[0–2]`, `embed` (src, title, height, crop, source, link). Figma: `Block/Split embed` `320:6274`, doc frame `319:6246` (built 21 Sep 2026; the iframe is a provider label on a cropped white frame).
- Text side cols 1–5 (Eyebrow, H2/H3, rich text, 0–2 actions), a framed iframe cols 7–12 (Position Start swaps the sides): the iframe cropped to `crop` px, with a source line + link below. Used for the ir.oms.no share graph (standardPage cropped to quote + chart until OMS supplies a compact module, Q60). Mobile: text, then the frame.
- **Open:** the frame loads straight away; production should route it through the Embed block's consent placeholder (§2.13) before launch (docs/09 §9).
- Review round 1 (from Split media): the text column stays one flow (eyebrow · H2 · one paragraph · one action; the heading carries the key message) and both sides are vertically centred, per Figma 162:2217.
...
- **Results style moved to its own block, §2.5b Results band (21 Sep 2026, Q67).** Stats band is now Plain · Panel · Feature.

### 2.5b Results band · `reach/results-band` (split out of Stats band, 21 Sep 2026, Q67)
`Breakpoint` = **2**; `Title`, `Currency`, `Published`, `Show published`. Fields: `title`, `published`, `currency`, `stats[4]` (headline), `secondary[4]`, `cards[]` (Report cards), `animate`. All read from the *Latest results* options page. Max one per page. Figma: `Block/Results band` `322:6434`, doc frame `321:6312` (built 21 Sep 2026; reuses the Report card set `294:2397`).
- **Built 18 Sep 2026 (Why invest):** latest reported quarter from the *Latest results* options page (`src/data/investor-results.ts`). The title (H2) and an "As reported" badge + publication date sit on one row. Below them: 4 headline figures (unit · value · scale, change line with a trend arrow) under a 2px `text/primary` rule, with 1px dividers between them, then 4 secondary figures in a ruled row. The footer has the next report (calendar tile, generated .ics "Add to calendar") and the report file link. Layout: 4-up, 2×2 below 1100, unit on its own line below 600. **Review rounds 2–3:** "All figures in {currency}" follows the title; there is one 1px rule, between the headline and secondary rows. Headline figures are 32 → 40, a step below the H2 title (round 4). The footer is a full-width tint strip flush with the section's bottom edge (it bleeds via box-shadow + clip-path, so there's no scrollbar), holding three evenly spaced items (no rules): the next report (calendar tile, .ics) plus 0–2 PDF downloads (latest quarterly report, annual report). **Round 7:** from 600 to 1099 it keeps three columns, each tile above its text; below 600 it becomes one compact column (tile beside text, 32 between items). Headline figures count up with decimals kept.
...
- **New token (19 Sep 2026): `text/accent-display`**: sage-500 in Light, sage-300 in Navy, for display-size figures (Card Stat, Split media Figures). Large text needs 3:1, so these can be one step lighter than `text/accent` (sage-600), closer to brand sage. Body-size accent text keeps sage-600.
- **Layout Track (19 Sep 2026, Investors overview):** the same milestones on a horizontal time axis. The window runs from 7 months back to 8 months ahead of today (15 months). Fields added: `isoDate` (drives position and status), `kind` primary (label above the rail) · secondary (label below), `short` (the visible label, e.g. Q4 25 · AR 25 · AGM; `title` stays the accessible name). All labels share one style: bold (primary) or medium (secondary) title and a regular `text/secondary` date. The markers do the highlighting, not the text. Month axis: short 6px ticks at the 1st of each month, all `border/default`, each label centred in its month's span; January and the first month read "Jan 27" / "Feb 26" in bold. The rail is drawn in `text/primary` up to a Today marker (`icon/accent`) and plain `border/default` after it. Done markers are filled; the newest done primary one is **Latest** (accent fill), the first upcoming one is **Next** (an accent ring that keeps breathing, solid, scale only). `panels` renders the shared **Report cards** under the track. Motion: the rail draws to today and each marker lands as the rail reaches it; stems and labels rise out of a clip, with no fades. Below 900 the track scrolls sideways (min 1200) and opens at today, and the panels stack. Data: `src/data/investor-calendar.ts`. New shared parts: **Date tile** (`src/components/DateTile.astro`) and **Report cards** (`src/components/ReportCards.astro`): three equal cards in date order, Annual report · Latest quarter · Next date (Navy, countdown, .ics). Each card has a tile, kicker, H4 and meta, then its 1–3 actions in a ruled row pinned to the foot. The Stats band Results style uses the same cards instead of its old tint footer strip (19 Sep 2026, so Why invest and the Overview match). Data: `reportCards` in `src/data/investor-results.ts`.
...
- **Panel + contact (18 Sep 2026):** the message (H2, lead, actions left-aligned under it) sits on the left, and the person is a card on the right behind a 1px rule: eyebrow label ("Investor relations"), name H4, role, then email and phone stacked. Below 1200 (raised from 900 on 20 Sep 2026: the panel squashed at ~900) the card goes under the actions, behind a top rule.
- **Short copy (Ross, 20 Sep 2026):** on every CTA panel or band the heading stays short and snappy and the body is one line, in the client PDF's own wording where it has one (Careers: "Ready to reach further?" / "Browse current openings and apply through our Candidate Portal."). A CTA is a nudge, not another text block.
...
### 2.16 Offices map · `reach/offices-map` (added 22 Sep 2026, Contact, Q73–Q75; outside the 14-block budget)
`Background` White · Tint · Navy × `Heading level` h1 (page opener) · h2 × `Breakpoint` = **Desktop built** (Mobile documented in code: 5:4 map edge to edge, one card column, the address card docks along the bottom of the map). Code: `src/blocks/OfficesMap.astro`; data `src/data/contact.ts` (every value sourced to the client PDF p59 or the dev site); the map itself is the shared dot-matrix map (`src/components/DotMap.astro`, `src/lib/dot-map-client.ts`, docs/07 §3 "Shared map").

- Where Reach's offices are: a full-width dot-matrix world map (the Live operations map, reused) with a solid pin per office and a hollow "quiet" pin per presence place (Sweden, Brazil, Cyprus: no card), then the eight offices as a 4 × 2 grid of tint cards (HQ first, PDF order) and a one-line "Also present in …" note.
- Section header can be the page's h1 (`headingLevel="h1"`, `first`: the block clears the fixed site header and opens the page with no Page hero, Q75). Cards then use h2, otherwise h3, so headings stay in order.
- Interaction: a pin click opens the office's address card on the map (kicker · city · address, phone, email meta rows · close), pans the map if the card would cross an edge, and lights the grid card; hovering a grid card pulses its pin once. Nearby offices cluster into one pin with a count ("5 offices"); clicking it fits every member in view and shows an "All offices" outline button that returns to the world view. No zoom buttons otherwise. Escape and a click on the map close the card; on phones the card docks along the bottom of the map.
- Reuses Section header, Card (Media None, Surface tint, eyebrow + title + Meta items), Meta item, Icon button (close). Dots: `DOT_PRESETS` regular, three width bands so the pitch matches Home's map (docs/07 §3).
- WordPress: an **Office** post type (or a Contact options repeater): name, label, country, address lines, phone, email, lat, lng, hq, type office | presence, order. The block queries it; the footer and header read the HQ row. Full proposal in the header of `src/data/contact.ts`.
- Contact: opens the page (`first`, h1 "Get in touch" + the PDF lead), then Card grid 3 columns icon media (the six topic mailboxes, library pictograms with loops), then Accordion Split (the PDF's three FAQs). No CTA band (the PDF has none; the footer carries phone, email and socials). The Contact list block drafted as §2.17 was dropped for the Card grid (Q75).
- Open items: Mobile not built in Figma; the Figma map is the same stylised dot field approach as Live operations, generated from the code's own poster this time (see the ledger entry).
...
| Investors overview | Hero Photo · Subnav Section pages · Timeline Track (investor year: Latest + Next) · Card bento lead-quad (Q2 at a glance: case, 3 stat cards, charter one-pager) · Split media Video + quote (CEO letter) · Split embed (live share graph) · Feed News Editorial · Accordion · CTA Panel (IR contact + Newsweb). Built 19 Sep 2026 |
| Why invest | **Built.** Hero Photo · Subnav Section pages · **Results band** (latest quarter) · **Figures** cards + sticky donut (four reasons) · **Split embed** (dividend, share graph) · Split media Numbered list stacked (three priorities) · Accordion Split on Tint · CTA panel on Tint |
| Reports & presentations | Hero Text · Subnav Section pages · Data list Reports · CTA Inline (IR contact + Newsweb). **TODO (19 Sep 2026):** annual report rows get an optional second file, ESEF (ZIP, the regulatory iXBRL filing, 2021 onward on the live site), shown as "ESEF (ZIP)" beside the PDF with hidden context ("Annual report 2025, ESEF format, ZIP"). Archive only, never on the Overview/Why invest report cards |
| Share information | Hero Text · Subnav · Embed Iframe × 3 (Q27) |
| About | **Built 20 Sep 2026 (`/company/`).** Hero Photo (vision) · Subnav Section pages · Split media **Wide** (Our story) · Stats band Feature on Tint · Card grid `strip` (certified management systems) · **Values** (line layout, Learn. Teach. Reach. and the payoff) · Split media Spec table (management team) · Accordion Split on Tint · CTA Band |
| Careers overview | **Built 20 Sep 2026 (`/careers/`, Q66).** Hero Photo (Animate, calm sea, "Open positions" to HR-Manager) · Subnav Section pages (Overview · Life at Reach · Our culture · Why work with us) · Split media **Wide without an image** ("Who thrives here": heading over two text columns, like About › Our story) (white) · Card grid 4 cols, icon media, numbered, `joined` to the block above (Learn · Teach · Reach + new **Never leave anyone behind** pictogram `value-behind`: white ground, tint cards) · Stats band Feature (people, countries, trainee programme since, trainees offered a full-time role; the PDF's stats panel, moved out of the hero) (tint) · Split media Image, position end (Trainees: workshop photo, the PDF's "Learn about the trainee program" link, target TBC) (white) · Embed 3D World Poster ("From ship to seabed": the world's careers route, `?careers=1`, launches in place; the poster is a still of the survey line's hoops from the world itself (`3d-world-careers-poster.jpg`, UI hidden, cropped to keep the hoops right of the copy); not in the PDF, added 21 Sep 2026) (white) · Card grid 4 cols, text cards (open positions: a **sample snapshot** of the HR-Manager list, replaced by the developer's feed; "All open positions" in the header) (tint) · Card bento trio-wide "More about working here" (PDF p. 49: three text cards to the child pages, then a wide 8 + narrow 4 pair of photo cards "Offshore & onshore, side by side" and "HSEQ first", the PDF's wording) (white) · Accordion Split, Careers FAQs (tint) · CTA Panel + named recruiter (tint). Grounds do not have to alternate white / tint / navy: where two blocks read as one, they share a ground. Copy and lists: `src/data/careers.ts`; figures from `key-figures.ts` (added `trainee-since`, `trainee-offer`). Card grid gained `joined` (drops its top padding under a same-ground block); Split media Wide now works without an image |
| Newsroom / News single | Hero Text + Meta row · core content · Feed News (related) |
| Contact | **Built 22 Sep 2026 (`/contact/`, Q73–Q75).** No hero: Offices map `first` with h1 "Get in touch" + the PDF lead (white; full-width dot-matrix map, 8 office cards 4 × 2 on tint, "Also present in" note) · Card grid 3 columns, icon media, white cards (the six topic mailboxes as pictogram cards, the mailbox as the link) (tint) · Accordion Split (PDF's three FAQs, "See all FAQs") (white). No CTA band. Every address, phone and email from `src/data/contact.ts`, each sourced; counts from key figures. Footer: Contact moved to the Get in touch column |
