...
_**21 Sep 2026 (Q67): three blocks split for editors.** Card grid's Bento → **Card bento** (§2.2b); Split media's Figures and Embed → **Figures** (§2.4b) and **Split embed** (§2.4c), Card media dropped; Stats band's Results → **Results band** (§2.5b). Nothing changed visually (checked element by element on the seven built pages at 1440 · 1100 · 800 · 375). Figma still has the old shapes: *Figma to add*. Block count in code: 22 (docs/09 §4)._
...
`Pattern` lead-quad · lead-tall-wide · lead-tall-trio-a · lead-tall-trio-b · trio-wide × `Breakpoint`. Fields: `sectionHeader`, `pattern`, `cards[]` (one per cell, in order). *Figma to add.*
...
`Layout` Sticky · Cards. Fields: eyebrow, title, `layout`, `figures[]` (kicker, title, text, figure or `split[]`, caption, period). *Figma to add.*
...
`Position` End · Start. Fields: eyebrow, title, `headingLevel`, `body`, `actions[0–2]`, `embed` (src, title, height, crop, source, link). *Figma to add.*
...
Fields: `title`, `published`, `currency`, `stats[4]` (headline), `secondary[4]`, `cards[]` (Report cards), `animate`. All read from the *Latest results* options page. Max one per page. *Figma to add.*
