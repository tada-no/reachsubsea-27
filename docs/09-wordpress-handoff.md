...
1. **The spec lags the code.** The code has **22** section blocks. docs/05 documents 19 (the four blocks split out in Q67 now have §2.2b, §2.4b, §2.4c, §2.5b). *Lifecycle*, *Values* and *Social feed* still have no §2 entry, and Figma has only the original 15. Several variants (Page hero Video, Card grid Strip, Stats Feature, CTA Panel, Split media Wide / Quote) are built and only partly written up ("Figma to add" flags in docs/05).
