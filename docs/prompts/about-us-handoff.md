...
- **JS:** an IntersectionObserver on a 1px line at the middle of the pinned band (its sticky `top` + half its height, re-measured on resize) sets `data-active` (0–2, the last value during the promise), `.is-promise`, `data-state` (past · current · future) on each figure subject and reel item, and adds `.is-playing` to the step's and figure's pictograms (the Reach subject replays when the promise takes over). `.is-live` gates every JS-only style.
...
- Not built (rejected 20 Sep): the pinned words stage, a fully pinned swap, and stacking sheets.
