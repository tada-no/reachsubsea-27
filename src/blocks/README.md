...
3. **CSS.** `<style is:global>` in the block file. Every selector starts with the block's root class, BEM inside (`.cta-band__actions`). Tokens only: no hex, no rgb, no raw spacing. Raw px only for fixed geometry the spec names (control heights, 16px markers, 1–2px rules, min-heights like 640/520, fixed column widths like 416/864 when the spec gives them).
